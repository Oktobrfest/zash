<?php
// File: app/Plugin/Webzash/View/Reports/cashflow2.ctp
?>
	...
	$(document).ready(function() {
		...
		$('#cashflow2Startdate').datepicker({
			...
			onClose: function(selectedDate) {
				if (selectedDate) {
					$("#cashflow2Enddate").datepicker("option", "minDate", selectedDate);
				} else {
					$("#cashflow2Enddate").datepicker("option", "minDate", startDate);
				}
			}
		});
		$('#cashflow2Enddate').datepicker({
			minDate: startDate,
			maxDate: endDate,
			dateFormat: '<?php echo Configure::read('Account.dateformatJS'); ?>',
			numberOfMonths: 1,
			onClose: function(selectedDate) {
				if (selectedDate) {
					$("#cashflow2Startdate").datepicker("option", "maxDate", selectedDate);
				} else {
					$("#cashflow2Startdate").datepicker("option", "maxDate", endDate);
				}
			}
		});
	});
...
<div id="accordion">
	<h3>Options</h3>
	<div class="cashflow2 form">
		<?php
		echo $this->Form->create('cashflow2', array(
			'inputDefaults' => array(
				'div' => 'form-group',
				'wrapInput' => false,
				'class' => 'form-control',
		...
		echo $this->Html->link(__d('webzash', 'Clear'), array('plugin' => 'webzash', 'controller' => 'reports', 'action' => 'cashflow2'), array('class' => 'btn btn-default'));
		echo '</div>';
...
<table class="stripped">
	...
	<tr>
		...
		<td></td>
		<td class="text-right"><?php echo toCurrency('D', $cashflow2['beginning_balance']); ?></td>
	</tr>
	...
	<?php foreach ($cashflow2['operating_activities'] as $item): ?>
		<tr>
	...
	<?php if (!empty($cashflow2['operating_subtotals'])): ?>
		<?php foreach ($cashflow2['operating_subtotals'] as $subtitle => $amount): ?>
			<tr>
	...
	<tr class="bold-text">
		...
		<td></td>
		<td class="text-right"><?php echo toCurrency('D', $cashflow2['totals']['operating']); ?></td>
	</tr>
	...
	<?php foreach ($cashflow2['investing_activities'] as $item): ?>
		<tr>
			<td><?php echo h($item['name']); ?></td>
	...
	<tr class="bold-text">
		...
		<td></td>
		<td class="text-right"><?php echo toCurrency('D', $cashflow2['totals']['investing']); ?></td>
	</tr>
	...
	<?php foreach ($cashflow2['financing_activities'] as $item): ?>
		<tr>
			<td><?php echo h($item['name']); ?></td>
			<td class="text-right"><?php echo isset($item['code']) ? h($item['code']) : ''; ?></td>
			<td class="text-right"><?php echo toCurrency($item['dc'], $item['amount']); ?></td>
	...
	<tr class="bold-text">
		...
		<td></td>
		<td class="text-right"><?php echo toCurrency('D', $cashflow2['totals']['financing']); ?></td>
	</tr>
	...
	<tr class="bold-text">
		...
		<td></td>
		<td class="text-right"><?php echo toCurrency('D', $cashflow2['net_change']); ?></td>
	</tr>

	<tr class="bold-text">
		<td>Ending Cash Balance</td>
		<td></td>
		<td class="text-right"><?php echo toCurrency('D', $cashflow2['ending_balance']); ?></td>
	</tr>
</table>
